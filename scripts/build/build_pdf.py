...
# ----------------- 경로 설정 -----------------
html_temp_path = str(paths.ROOT_DIR / "scratch" / "temp_storybook.html")
...
def main():
    ...
    if unit in UNIT_CONFIGS:
        ...
        # Autodetect
        stories_grade_path = str(paths.story_dir(grade_dir))
        script_name = None
        ...
        assets_parent = str(paths.APPS_DIR / "assets")
        assets_folder = None
    ...
    script_path = str(paths.story_path(grade_dir, script_name))
    if not os.path.exists(script_path):
        script_path = str(paths.ARCHIVE_DIR / "releases" / script_name)
        
    assets_dir = str(paths.APPS_DIR / "assets" / assets_folder)
    pdf_output_path = str(paths.pdf_output_path(grade_dir, unit))
...
def compile_pdf_for_unit(unit_id: str) -> bool:
    """
    [Programmatic API] 특정 단원의 Generated 스토리보드를 읽어 PDF 대본집을 컴파일합니다.
    이미 최신 상태의 스토리보드가 storyboards/generated/에 존재한다는 전제하에 동작합니다.
    ...
    # argparse 없이 인자를 강제 주입하여 main() 로직 재사용
    saved_argv = sys.argv[:]
    sys.argv = [sys.argv[0], '--unit', unit_id]
    try:
        main()
        return True
