...
THEMES = {
    "01": {"accent": "#a855f7", "accent_hover": "#c084fc", "bg": "#120a1d", "glass": "rgba(25, 15, 38, 0.75)", "border": "rgba(168, 85, 247, 0.25)"},
    "02": {"accent": "#10b981", "accent_hover": "#34d399", "bg": "#061a12", "glass": "rgba(10, 30, 20, 0.75)", "border": "rgba(16, 185, 129, 0.25)"},
    "03": {"accent": "#2563eb", "accent_hover": "#60a5fa", "bg": "#060f1e", "glass": "rgba(10, 20, 35, 0.75)", "border": "rgba(37, 99, 243, 0.25)"},
    "04": {"accent": "#6366f1", "accent_hover": "#818cf8", "bg": "#0d0e22", "glass": "rgba(18, 19, 45, 0.75)", "border": "rgba(99, 102, 241, 0.25)"},
    "05": {"accent": "#f59e0b", "accent_hover": "#fbbf24", "bg": "#1c140a", "glass": "rgba(35, 25, 15, 0.75)", "border": "rgba(245, 158, 11, 0.25)"},
    "06": {"accent": "#e11d48", "accent_hover": "#fb7185", "bg": "#1e0b10", "glass": "rgba(35, 15, 20, 0.75)", "border": "rgba(225, 29, 72, 0.25)"},
    "07": {"accent": "#06b6d4", "accent_hover": "#22d3ee", "bg": "#06181e", "glass": "rgba(10, 28, 35, 0.75)", "border": "rgba(6, 182, 212, 0.25)"}
}

# Subtitle naming mapping for unit H2 titles
SUBTITLES = {
    "01": "제곱근과 실수",
    "02": "다항식의 곱셈과 인수분해",
    "03": "이차방정식",
...
def make_ans_check(ans_val):
    ans_clean = ans_val.replace(" ", "").upper()
    if ans_clean == '1/2':
        return "ans === '1/2' || ans === '0.5'"
    elif ans_clean == '1/3':
        return "ans === '1/3'"
    elif ans_clean == '1/4':
        return "ans === '1/4' || ans === '0.25'"
    elif ans_clean == '3/4':
        return "ans === '3/4' || ans === '0.75'"
    elif ans_clean == '1/5':
        return "ans === '1/5' || ans === '0.2'"
    elif ans_clean == '2/5':
        return "ans === '2/5' || ans === '0.4'"
    elif ans_clean == '3/5':
        return "ans === '3/5' || ans === '0.6'"
    elif ans_clean == '4/5':
        return "ans === '4/5' || ans === '0.8'"
    elif '±' in ans_clean:
        num = ans_clean.replace('±', '')
        return f"ans === '±{num}' || ans === '+-{num}' || ans === '{num},-{num}' || ans === '-{num},{num}'"
    return f"ans === '{ans_clean}'"

def parse_txt_file(filepath):
    with open(filepath, 'r', encoding='utf-8') as f:
        lines = f.readlines()
        
    episode_title = "방탈출 게임"
    intro_story = "미션을 해결하여 방을 탈출하세요!"
    outro_story = "모든 방을 무사히 탈출했습니다!"
    ...
    for line in lines:
        ...
        # Parse Episode Title
        if line_strip.startswith("# 에피소드"):
            match = re.match(r"# 에피소드\s*\d+\.\s*(.*)", line_strip)
            if match:
                episode_title = match.group(1).strip()
            continue
            
        # Identify sections
        if line_strip.startswith("## 2. [교사용 오프닝 스크립트]"):
            current_section = "opening"
            continue
        elif line_strip.startswith("## 3. [학생용 미션 지문]"):
            current_section = "questions"
            continue
        elif line_strip.startswith("## 4. [교사용 진행 가이드 및 힌트]"):
            current_section = "hints"
            continue
        elif line_strip.startswith("## 5. [정답 및 해설]"):
            current_section = "answers"
            continue
        elif line_strip.startswith("## 6. [교사용 엔딩 스크립트]"):
            current_section = "ending"
            continue
            
        # Parse section contents
        if current_section == "opening":
            # Extract opening story inside quotes
            match = re.search(r'"([^"]+)"', line_strip)
            if match:
                intro_story = match.group(1).strip()
            elif not line_strip.startswith("##") and len(line_strip) > 10:
                intro_story = line_strip.strip()
        elif current_section == "ending":
            # Extract ending story inside quotes
            match = re.search(r'"([^"]+)"', line_strip)
            if match:
                outro_story = match.group(1).strip()
            elif not line_strip.startswith("##") and len(line_strip) > 10:
                outro_story = line_strip.strip()
        elif current_section == "questions":
            # Parse questions: - **Q1.** 내용
            match = re.match(r"-\s*\*\*Q(\d+)\.\*\*\s*(.*)", line_strip)
            if match:
                qn = int(match.group(1))
                qtext = match.group(2).strip()
                questions[qn] = qtext
        elif current_section == "hints":
            # Parse hints: - **Q1 힌트:** 내용
            match = re.match(r"-\s*\*\*Q(\d+)\s*힌트:\*\*\s*(.*)", line_strip)
            if match:
                qn = int(match.group(1))
                hint_text = match.group(2).strip()
                hints[qn] = hint_text
        elif current_section == "answers":
            # Parse answers: - Q1: 정답 (설명)
            match = re.match(r"-\s*Q(\d+):\s*(.*)", line_strip)
            if match:
                qn = int(match.group(1))
                ans_raw = match.group(2).strip()
                # Split brackets if present
                if '(' in ans_raw:
                    ans_raw = ans_raw.split('(')[0].strip()
                answers[qn] = ans_raw
    ...
    for qn in sorted(questions.keys()):
        qtext = questions[qn]
        hint = hints.get(qn, "스토리 지문과 힌트를 다시 한번 분석해보세요.")
        ans = answers.get(qn, "Y")
        
        # Clean question text and extract HTML strong tag
        # Add basic placeholder
        placeholder = "정답 입력"
        if "영어 기호" in qtext or "합동 조건" in qtext:
            placeholder = "영문 대문자 입력"
        elif "분수" in qtext or "비례식" in qtext:
        ...
        qs.append({
            ...
            "qtext": f"<strong>Q{qn}.</strong> {qtext}",
            "placeholder": placeholder,
            "error": "정답이 올바르지 않습니다. 다시 계산해보세요.",
            "ans_check": make_ans_check(ans),
            "hint": hint
        })
        
    return episode_title, intro_story, outro_story, qs

# Template code for generating update_app_m3_{num}.py
# Use custom replacement tags instead of {} to avoid format() KeyError
BUILDER_TEMPLATE = """import re
import os

current_dir = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.dirname(os.path.dirname(current_dir))
apps_dir = os.path.join(project_root, "apps")
html_file = os.path.join(apps_dir, "app_m3_[[NUM]]_escape_room.html")
html_path = html_file
...
<head>
    ...
    <title>[[EPISODE_TITLE]]: 방탈출 게임</title>
    ...
    <style>
        :root {
            --bg-main: [[THEME_BG]];
            --glass-bg: [[THEME_GLASS]];
            --glass-border: [[THEME_BORDER]];
            --accent: [[THEME_ACCENT]];
        ...
        * {
        ...
        body::before {
            ...
            background: 
                radial-gradient(circle at 10% 20%, rgba([[RGBA_ACCENT]], 0.08) 0%, transparent 40%),
        ...
        .glass-panel {
            ...
            border-top: 1px solid rgba([[RGBA_ACCENT]], 0.4);
            border-left: 1px solid rgba([[RGBA_ACCENT]], 0.4);
            border-radius: 24px;
            padding: 3rem;
            box-shadow: 0 0 50px rgba([[RGBA_ACCENT]], 0.15), inset 0 0 20px rgba([[RGBA_ACCENT]], 0.02);
            display: none; 
            opacity: 0;
        ...
        h1 {
            ...
            text-shadow: 0 0 30px rgba([[RGBA_ACCENT]], 0.3);
            letter-spacing: 2px;
        }
        ...
        h2 {
            ...
            font-weight: 500;
            letter-spacing: 1px;
            border-bottom: 1px solid rgba([[RGBA_ACCENT]], 0.15);
        ...
        .story-box {
            ...
            border: 1px solid rgba([[RGBA_ACCENT]], 0.15);
            border-radius: 12px;
            padding: 1.25rem;
            margin-bottom: 1.5rem;
            position: relative;
        ...
        .question-box {
            background: rgba([[RGBA_ACCENT]], 0.05);
            border: 1px dashed rgba([[RGBA_ACCENT]], 0.3);
            border-radius: 12px;
            padding: 1.5rem;
            margin-bottom: 1.5rem;
        }
        ...
        input[type="text"] {
            ...
            border: 1px solid rgba([[RGBA_ACCENT]], 0.3);
            border-radius: 8px;
        ...
        input[type="text"]:focus {
            ...
            box-shadow: 0 0 10px rgba([[RGBA_ACCENT]], 0.3);
        }
        ...
        .btn {
            ...
            cursor: pointer;
            transition: all 0.3s;
            box-shadow: 0 4px 15px rgba([[RGBA_ACCENT]], 0.3);
            letter-spacing: 1px;
        }

        .btn:hover {
            transform: translateY(-2px);
            box-shadow: 0 6px 20px rgba([[RGBA_ACCENT]], 0.5);
        ...
        .progress-container {
            ...
            border: 1px solid rgba([[RGBA_ACCENT]], 0.2);
            border-radius: 50px;
            height: 10px;
            margin-bottom: 2rem;
            overflow: hidden;
        ...
        .log-content h2 {
            border-bottom: 1px solid rgba([[RGBA_ACCENT]], 0.2);
...
<body>
    ...
    <div class="container">
        ...
        <div id="intro" class="glass-panel active">
            <h1>[[EPISODE_TITLE]]</h1>
            <h2>[[SUBTITLE]]</h2>
            <img src="https://jk1027.github.io/room-math-story/apps/assets/m3_[[NUM]]/intro.png" alt="Background" class="panel-image">
            <div class="story-box">
                <div class="story-text">[[INTRO_STORY]]</div>
            </div>
        ...
        <div id="outro" class="glass-panel">
            <h1>미션 완료!</h1>
            <h2>최종 봉인 탈출 성공</h2>
            <img src="https://jk1027.github.io/room-math-story/apps/assets/m3_[[NUM]]/outro.png" alt="Ending" class="panel-image">
            <div class="story-box">
                <div class="story-text">[[OUTRO_STORY]]</div>
            </div>
            <div class="btn-group">
...
for q in qs:
    ...
    panel = f'''
        ...
        <div id="panel_q{qnum}" class="glass-panel">
            ...
            <img src="https://jk1027.github.io/room-math-story/apps/assets/m3_[[NUM]]/q{qnum}.png" alt="Background" class="panel-image">
            <div class="story-box">
...
print("app_m3_[[NUM]]_escape_room.html generated successfully.")
"""
...
def main():
    # Find all txt files in stories/중3/
    txt_files = glob.glob(os.path.join(stories_dir, "m3_*.txt"))
    ...
    for filepath in txt_files:
        ...
        # Parse unit number
        match = re.match(r"m3_(\d+)_.*\.txt", filename)
        if not match:
            continue
            
        unit_num = match.group(1)
        print(f"Processing unit {unit_num} ({filename})...")
        
        # Parse content
        episode_title, intro_story, outro_story, qs = parse_txt_file(filepath)
        
        # Get theme
        theme = THEMES.get(unit_num, {"accent": "#00f0ff", "accent_hover": "#33f3ff", "bg": "#0c101b", "glass": "rgba(15, 23, 42, 0.75)", "border": "rgba(224, 180, 76, 0.25)"})
        subtitle = SUBTITLES.get(unit_num, "수학 방탈출")
        rgba_accent = hex_to_rgb_str(theme["accent"])
        
        # Format builder file contents using replace instead of format() to avoid braces KeyError
        import json
        qs_json = json.dumps(qs, indent=4, ensure_ascii=False)
        
        builder_content = BUILDER_TEMPLATE
        builder_content = builder_content.replace("[[NUM]]", unit_num)
        builder_content = builder_content.replace("[[EPISODE_TITLE]]", episode_title)
        builder_content = builder_content.replace("[[SUBTITLE]]", subtitle)
        builder_content = builder_content.replace("[[THEME_BG]]", theme["bg"])
        builder_content = builder_content.replace("[[THEME_GLASS]]", theme["glass"])
        builder_content = builder_content.replace("[[THEME_BORDER]]", theme["border"])
        builder_content = builder_content.replace("[[THEME_ACCENT]]", theme["accent"])
        builder_content = builder_content.replace("[[THEME_ACCENT_HOVER]]", theme["accent_hover"])
        builder_content = builder_content.replace("[[RGBA_ACCENT]]", rgba_accent)
        builder_content = builder_content.replace("[[INTRO_STORY]]", intro_story)
        builder_content = builder_content.replace("[[OUTRO_STORY]]", outro_story)
        builder_content = builder_content.replace("[[QS_PLACEHOLDER]]", qs_json)
        
        # Save builder file to scripts/builders/update_app_m3_{num}.py
        builder_filename = f"update_app_m3_{unit_num}.py"
        builder_filepath = os.path.join(builders_dir, builder_filename)
        
        with open(builder_filepath, 'w', encoding='utf-8') as f:
            f.write(builder_content)
            
        print(f"Generated builder script: {builder_filename}")
