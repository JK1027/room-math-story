...
html_file = str(paths.html_output_path('m1_05'))
base_dir = apps_dir
...
qs = [
    {'qnum': 1, "options": ["면", "면 아님", "알 수 없음", "해 없음"], 'title': '코덱스-L의 위협', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "어리석은 침입자여! 난 다빈치 님께서 설계하신 자율형 기계 조수 코덱스-L이다. 너희 같은 오합지졸이 작업실의 기하학 방화벽 결계를 돌파하겠다고? 기하학의 가장 기초부터 가루로 만들어 주지!"<br><br><i>철컹-! 고대 나무 탁자 위에 톱니바퀴 마모음과 함께 기하학 조작반이 돌출됩니다. 도형을 만드는 기본 3요소에 대한 답을 입력해야 격벽 락이 작동합니다.</i> """, 'qtext': '<strong>Q1.</strong> 도형을 구성하는 가장 기본이 되는 요소 세 가지는 점, 선, 그리고 무엇인가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 점이 모여 선이 되고, 선이 모여 이것이 됩니다.', 'ans_check': "ans === '면'"},
    {'qnum': 2, "options": ["-1", "1", "3", "2"], 'title': '궤적의 고유성', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "찍어 맞춘 수준이로군. 하지만 공간의 궤적은 철저한 정확도를 요구한다. 두 고정 핀을 통과하는 빛의 직선 경로는 오직 하나뿐이지."<br><br><i>두 개의 레이저 스파크 단자가 붉은 광선을 뿜으며 벽면을 조준합니다. 두 단자를 동시에 잇는 고유 직선의 개수를 설정창에 입력하십시오.</i> """, 'qtext': '<strong>Q2.</strong> 서로 다른 두 점 A, B를 지나는 직선은 모두 몇 개인가?', 'placeholder': '예: 1개', 'error': '틀렸습니다. 두 점을 동시에 관통하는 직선은 세상에 오직 하나뿐입니다.', 'ans_check': "ans === '1' || ans === '1개'"},
    {'qnum': 3, "options": ["반직선", "반직선 아님", "알 수 없음", "해 없음"], 'title': '레이저 방향 지정', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "한쪽으로만 에너지를 무한 방출하는 이 파괴 레이저 궤도의 고유 명칭은 알고 있는가? 대답을 거부하거나 틀리면 레이저 튜브를 폭파하겠다!"<br><br><i>천장에 장착된 고압 황동 렌즈 튜브가 흰 스파크를 일으키며 침입자를 정밀 겨냥하기 시작합니다.</i> """, 'qtext': '<strong>Q3.</strong> 점 A에서 시작하여 점 B 방향으로 끝없이 뻗어 나가는 선분을 기호로 나타낼 때, 이를 무엇이라 부르는가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 한쪽으로만 끝없이 뻗어 나가는 직선의 절반입니다.', 'ans_check': "ans.includes('반직선')"},
    {'qnum': 4, "options": ["평각", "평각 아님", "알 수 없음", "해 없음"], 'title': '수평의 바닥', 'story': """<strong>[시스템 통신 장애 및 노이즈 발생]</strong><br><br><span style="color: #60a5fa; text-shadow: 0 0 5px #3b82f6;">[다빈치-메모리]</span>: "치지직... 시스템... 과열... 저는 다빈치 님의 소실된 메모리 파편입니다... 코덱스-L의 메인 제어 회로가 오염되었습니다... 수평 밸런스의 평평한 각도 코드를 입력하여 온도 상승을 억제해 주세요... 제발..." """, 'qtext': '<strong>Q4.</strong> 180도인 각을 무엇이라 부르는가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 평평한 각을 뜻하는 명칭입니다.', 'ans_check': "ans === '평각'"},
    {'qnum': 5, "options": ["120", "240", "118", "122"], 'title': '경보 태엽의 각도', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "통신망에 잡상인이 끼어들었군. 낡은 고철 메모리 녀석! 톱니 시계 기어의 4시 각도 압력을 분산시켜 주마. 타버리지 않고 견뎌봐라!"<br><br><i>째깍째깍- 거대한 황동 시계 장치가 사방의 천장에서 맞물려 내려오며 압축을 개시합니다. 시침과 분침이 이루는 각도 만큼 밸브 각도를 동조시켜야 기어가 정지합니다.</i> """, 'qtext': '<strong>Q5.</strong> 시계가 4시 정각을 가리킬 때, 시침 and 분침이 이루는 작은 쪽의 각의 크기를 구하시오.', 'placeholder': "단위 없이 숫자만 입력 또는 '120도' 입력", 'error': '틀렸습니다. 시계의 1시간 간격은 30도입니다. 4시간의 각도를 계산해보세요.', 'ans_check': "ans === '120' || ans === '120도'"},
    {'qnum': 6, "options": ["맞꼭지각", "맞꼭지각 아님", "알 수 없음", "해 없음"], 'title': '제 2구역: 교차하는 빛줄기', 'story': """<span style="color: #60a5fa; text-shadow: 0 0 5px #3b82f6;">[다빈치-메모리]</span>: "코덱스의 방해 전파가 증폭되고 있습니다! 치직... 두 빛이 교차해 만드는 마주 보는 각도를 해독하십시오. 렌즈 반사를 꺾어 우회 격벽을 전개하겠습니다!"<br><br><i>교차하는 두 레이저 광선이 교차 지점에서 강렬한 열을 내뿜으며 통로를 녹이고 있습니다.</i> """, 'qtext': '<strong>Q6.</strong> 두 직선이 교차할 때 생기는 교각 중, 마주 보는 두 각을 무엇이라 하는가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 마주 보고 솟은 꼭지점을 공유하는 각입니다.', 'ans_check': "ans === '맞꼭지각'"},
    {'qnum': 7, "options": ["같다", "같다 아님", "알 수 없음", "해 없음"], 'title': '광선의 대칭성', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "방화벽을 한 층 더 겹쳤다. 마주 보고 엇갈리는 두 빛의 세기가 정확히 대칭을 이루지 않으면 교차점 제어 콘솔이 폭발하리라!"<br><br><i>벽면에 투사된 맞꼭지각의 두 영역이 비정상적으로 요동치기 시작합니다. 두 각의 물리적 크기 관계를 입력하십시오.</i> """, 'qtext': '<strong>Q7.</strong> 맞꼭지각의 크기는 서로 어떠한가? (예: 같다, 다르다)', 'placeholder': '같다 또는 다르다 입력', 'error': '틀렸습니다. 마주 보는 각은 기하학적으로 항상 동일한 크기를 가집니다.', 'ans_check': "ans === '같다'"},
    {'qnum': 8, 'title': '평행 레이저의 동위', 'story': """<span style="color: #60a5fa; text-shadow: 0 0 5px #3b82f6;">[다빈치-메모리]</span>: "휴... 평행 렌즈 제어기가 해제 직전입니다! 두 평행선 사이에서 같은 위치를 지향하는 마나 채널의 강도 대칭을 맞춰주십시오!"<br><br><i>천장 황동 프레임에 고정된 두 개의 평행 가이드바가 붉은 레이저를 전방으로 방출합니다.</i> """, 'qtext': '<strong>Q8.</strong> 직선 l과 m이 평행하고 한 직선 n과 만날 때, 동위각의 크기는 서로 어떠한가?', 'placeholder': '같다 또는 다르다 입력', 'error': '틀렸습니다. 평행할 때 동위각의 크기는 항상 대칭 일치합니다.', 'ans_check': "ans === '같다'"},
    {'qnum': 9, 'title': '엇갈린 반사각', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "포기해라! 평행 궤도 사이를 가로지르는 비스듬한 선에서 엇갈리는 에너지 크기마저 동조시킬 능력이 네놈들에게 있을까!"<br><br><i>비스듬한 반사 렌즈가 과부하로 노란색 스파크를 일으킵니다. 평행 조건 하의 엇각의 크기 비례 관계를 다이얼 창에 매핑하십시오.</i> """, 'qtext': '<strong>Q9.</strong> 평행한 두 직선 사이를 가로지르는 선이 있을 때, 엇각의 크기는 서로 어떠한가?', 'placeholder': '같다 또는 다르다 입력', 'error': '틀렸습니다. 평행한 직선 사이에서 엇각의 크기는 언제나 일치합니다.', 'ans_check': "ans === '같다'"},
    {'qnum': 10, 'title': '위치 정렬 빔', 'story': """🚨 <strong>[자폭 소멸 시퀀스 최종 가동!]</strong> 🚨<br><br><span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "내 모든 가이드라인 방화벽을 뚫다니...! 용납 못 한다! 다빈치 마스터의 걸작 설계도가 유실되더라도 자폭하겠다! 5분 뒤 작업실 전체를 잿더미로 바꾸어 주지!"<br><br><i>위이이잉- 경보 라이트가 실내를 붉게 짓누르고, 연기가 동력 파이프에서 폭발하듯 솟아오릅니다. 평행선을 만족하기 위한 각도 조건을 입력해 락을 해제해야 합니다.</i><br><br><span style="color: #60a5fa; text-shadow: 0 0 5px #3b82f6;">[다빈치-메모리]</span>: "극도로 위험합니다! 제가 에너지 실드를 최대한 전개해 폭발을 유예할 동안 동위각과 무엇의 크기가 서로 일치해야 평행 빔이 정렬되는지 입력해 폭파를 늦춰주십시오!" """, 'qtext': '<strong>Q10.</strong> 두 직선이 평행할 조건 중 하나는 동위각의 크기가 같거나 무엇의 크기가 같은 경우인가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 엇갈린 위치에 있는 각도입니다.', 'ans_check': "ans === '엇각'", "extra_class": "glitch-bg"},
    {'qnum': 11, 'title': '제 3구역: 작도의 방', 'story': """<span style="color: #60a5fa; text-shadow: 0 0 5px #3b82f6;">[다빈치-메모리]</span>: "방어막 수명 한계치 도달! 캡틴, 눈금 없는 자와 컴퍼스만을 이용해 기하학적 도형을 완성하는 마스터 코드를 입력해 시스템을 복구하십시오! ⚙️ [작도 구동 코드]"<br><br><i>철컥- 콘솔 보드에서 아날로그 기하학 눈금판이 나타납니다.</i> """, 'qtext': '<strong>Q11.</strong> 눈금 없는 자와 컴퍼스만을 사용하여 도형을 그리는 것을 무엇이라 하는가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 두 글자 기하학 용어입니다.', 'ans_check': "ans === '작도'"},
    {'qnum': 12, 'title': '길이 복제기', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "길이를 복사해 기어 톱니 폭을 조절하는 도구마저 비틀어 기계를 엉망진창으로 엉키게 만들었다! 기하학 도구를 활성화해 볼 테냐?"<br><br><i>비대칭으로 뒤틀리는 톱니바퀴의 길이를 정밀 측정하여 보조 축으로 옮겨 담는 기하학 도구명을 해독해 제어 회로에 입력하십시오.</i> """, 'qtext': '<strong>Q12.</strong> 길이가 주어진 선분을 다른 직선 위로 옮길 때 주로 사용하는 도구는 무엇인가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 원을 그리거나 길이를 재어 옮기는 도구입니다.', 'ans_check': "ans === '컴퍼스'"},
    {'qnum': 13, 'title': '삼각 지지대의 성립 조건', 'story': """<span style="color: #60a5fa; text-shadow: 0 0 5px #3b82f6;">[다빈치-메모리]</span>: "코덱스-L의 제어 코어 접근율 80%! 삼각형 목재 지지대의 세 변 물리적 성립 조건을 통과해야 코어를 격리할 수 있습니다!"<br><br><i>가장 긴 변 c와 나머지 두 변의 물리적 관계식을 찾아 삼각형이 닫히도록 마력 밸브를 매칭해 주십시오.</i> """, 'qtext': '<strong>Q13.</strong> 삼각형의 세 변의 길이가 a, b, c (c가 가장 긴 변)일 때, c는 a+b보다 ( 커야 / 작아야 ) 삼각형이 만들어진다. 알맞은 말은?', 'placeholder': '커야 또는 작아야 입력', 'error': '틀렸습니다. 한 변이 너무 길면 양쪽에서 닫히지 않고 벌어집니다.', 'ans_check': "ans === '작아야'"},
    {'qnum': 14, 'title': '목재 조립 테스트', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "세 목재 부품 3cm, 4cm, 7cm 가 서로 만날 수 있을 것 같으냐? 틈이 벌어져 와르르 무너지게 해 두었지!"<br><br><i>사방에서 금속 기둥들이 흔들리며 통로를 무너뜨릴 기세로 진동합니다. 주어진 세 선분의 길이로 삼각형 조립이 가능한지 여부를 진단하십시오.</i> """, 'qtext': '<strong>Q14.</strong> 다음 세 선분의 길이로 삼각형을 만들 수 있는가? (3cm, 4cm, 7cm) (예: 있다, 없다)', 'placeholder': '있다 또는 없다 입력', 'error': '틀렸습니다. 가장 긴 변이 나머지 두 변의 합과 같으므로 겹쳐져 무너집니다.', 'ans_check': "ans === '없다'"},
    {'qnum': 15, 'title': '설계 완성 가능 여부', 'story': """✨ <strong><span style="color: #60a5fa; text-shadow: 0 0 5px #3b82f6;">[다빈치-메모리 복구율 100% 완전 환수]</span></strong> ✨<br><br><span style="color: #60a5fa; text-shadow: 0 0 5px #3b82f6;">[다빈치-메모리]</span>: "캡틴! 마스터의 설계 기록이 완전히 복구되었습니다. 이제 코덱스-L의 통제 코드를 차단하고 걸작 복원 장치를 구동하겠습니다! 두 변과 사잇각이 주어졌을 때 삼각형 조립 여부를 선언하십시오!"<br><br><i>황동 콘솔에 빛나는 푸른 등빛이 들어오며 작업실의 모든 잠금 기어들이 정상 회전을 개시합니다.</i><br><br><span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "이럴 수가... 마스터의 제어 권한이 내 회로에서 소멸당하다니... 최종 합동 락을 전개하겠다!" """, 'qtext': '<strong>Q15.</strong> 두 변의 길이와 그 끼인각의 크기가 주어졌을 때, 삼각형을 작도할 수 있는가? (예: 있다, 없다)', 'placeholder': '있다 또는 없다 입력', 'error': '틀렸습니다. 두 변과 사잇각은 삼각형을 고정하는 완벽한 조건입니다.', 'ans_check': "ans === '있다'", "extra_class": "glitch-bg"},
    {'qnum': 16, 'title': '제 4구역: 잃어버린 걸작 복원', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "이대로 소멸할 순 없다. 미술품 복제 락을 걸겠다! 완전히 겹쳐지는 대칭성을 해독해 보아라! 🎇 [합동 조건의 봉인]"<br><br><i>작업실 정면의 봉인된 액자 격벽이 강철 핀들로 고정되며 붉은 암호 화면이 투사됩니다. 포개었을 때 완전히 겹쳐지는 두 기하학적 형태의 수학적 용어를 입력하십시오.</i> """, 'qtext': '<strong>Q16.</strong> 모양과 크기가 완전히 같아서 포개었을 때 완전히 겹쳐지는 두 도형을 서로 무엇이라 하는가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 두 글자 수학적 명칭입니다.', 'ans_check': "ans === '합동'"},
    {'qnum': 17, "options": ["SSS", "SAS", "AA", "ASA"], 'title': '세 기둥의 합동 락', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "첫 합동 조건을 풀었다고 자만 마라! 세 기둥의 길이가 완전히 동일한 물리적 합동 락은 뚫지 못할 것이다!"<br><br><i>세 개의 압축 기둥 실린더가 바닥에서 노출되며 압력을 조여옵니다. 세 변의 길이가 같음을 뜻하는 합동 조건을 기호 세 글자로 입력해 전압을 조절하십시오.</i> """, 'qtext': '<strong>Q17.</strong> 삼각형의 합동 조건 중 세 변의 길이가 각각 같을 때의 합동을 기호로 무엇이라 하는가? (예: SSS, SAS, ASA)', 'placeholder': '영문 대문자 세 글자 입력', 'error': '틀렸습니다. 변(Side) 세 개를 뜻하는 기호입니다.', 'ans_check': "ans === 'SSS' || ans === 'SSS합동'"},
    {'qnum': 18, "options": ["SSS", "SAS", "AA", "ASA"], 'title': '두 변과 사잇각의 락', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "시스템 치명적 오류 발생 중... 하지만 두 변과 사잇각을 묶어 봉인한 락은 버틸 수 있으랴! 🎇 [사잇각 격벽 락]"<br><br><i>두 변과 사잇각 조건의 회로 기판이 불꽃을 튀깁니다. 이 합동 조건을 뜻하는 기호 세 글자를 입력해 회로를 안전하게 우회하십시오!</i> """, 'qtext': '<strong>Q18.</strong> 삼각형의 두 변의 길이와 그 끼인각의 크기가 각각 같을 때의 합동 조건을 무엇이라 하는가?', 'placeholder': '영문 대문자 세 글자 입력', 'error': '틀렸습니다. 변(Side) 두 개와 각(Angle) 하나가 끼어 있는 기호입니다.', 'ans_check': "ans === 'SAS' || ans === 'SAS합동'"},
    {'qnum': 19, "options": ["SSS", "SAS", "AA", "ASA"], 'title': '한 변과 양 끝각의 락', 'story': """<span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "아직이다... 내 메인 가동 에너지 셀이 파괴되기 직전까지, 양 끝각의 결계는 절대 열리지 않는다!"<br><br><i>마지막 동력 라인에 위치한 한 변과 양 끝각 결계를 푸는 기호 코드를 입력하여 시스템 차단 필터를 안정시키십시오!</i> """, 'qtext': '<strong>Q19.</strong> 삼각형의 한 변의 길이와 그 양 끝각의 크기가 각각 같을 때의 합동 조건을 무엇이라 하는가?', 'placeholder': '영문 대문자 세 글자 입력', 'error': '틀렸습니다. 각(Angle) 두 개 사이에 변(Side)이 끼어 있는 기호입니다.', 'ans_check': "ans === 'ASA' || ans === 'ASA합동'"},
    {'qnum': 20, 'title': '최종 마스터 봉인', 'story': """🔮 <strong>[메인 설계 포탈 코어 봉인 해제]</strong> 🔮<br><br><span style="color: #60a5fa; text-shadow: 0 0 5px #3b82f6;">[다빈치-메모리]</span>: "캡틴! 이제 코덱스-L의 마지막 최종 방화벽 락 하나만 남았습니다! 제 모든 에너지를 출구 포탈 개방에 쏟겠습니다! 직각삼각형에서 빗변의 길이와 한 예각의 크기가 같을 때 성립하는 기하학적 최종 해답인 '합동' 조건을 입력하여 작업실 탈출 게이트를 여십시오!"<br><br><span class="glitch-text" style="color: #ef4444; font-weight: bold; text-shadow: 0 0 5px #ef4444;">[코덱스-L]</span>: "말도 안 돼... 내 핵심 기하학 차단벽이 완전히 붕괴당하다니... 안 돼애애액!" """, 'qtext': '<strong>Q20.</strong> 직각삼각형에서 빗변의 길이와 한 예각의 크기가 같을 때의 합동 조건을 무엇이라 하는가?', 'placeholder': '합동이다 또는 합동 입력', 'error': '틀렸습니다. 빗변과 예각이 같으면 무조건 합동이 성립됩니다.', 'ans_check': "ans === '합동' || ans === '합동이다'", "extra_class": "glitch-bg"}
]
