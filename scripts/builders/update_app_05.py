...
qs = [
    {'qnum': 1, "options": ["면", "면 아님", "알 수 없음", "해 없음"], 'title': '코덱스-L의 위협', 'story': '[코덱스-L]: "어리석은 침입자여! 난 다빈치 님께서 설계하신 기계 비서 코덱스-L이다. 너 같은 오합지졸이 내 기하학 결계를 풀 수 있을 것 같으냐? 기하학의 가장 기초부터 시작해보지!"', 'qtext': '<strong>Q1.</strong> 도형을 구성하는 가장 기본이 되는 요소 세 가지는 점, 선, 그리고 무엇인가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 점이 모여 선이 되고, 선이 모여 이것이 됩니다.', 'ans_check': "ans === '면'"},
    {'qnum': 2, "options": ["-1", "1", "3", "2"], 'title': '궤적의 고유성', 'story': '[코덱스-L]: "호오, 찍어서 맞췄나 보군. 하지만 두 개의 고정 핀 A, B를 잇는 직선의 궤적은 정확한 수치를 요구한다. 멍청한 두뇌로 계산할 수나 있으려나?"', 'qtext': '<strong>Q2.</strong> 서로 다른 두 점 A, B를 지나는 직선은 모두 몇 개인가?', 'placeholder': '예: 1개', 'error': '틀렸습니다. 두 점을 동시에 관통하는 직선은 세상에 오직 하나뿐입니다.', 'ans_check': "ans === '1' || ans === '1개'"},
    {'qnum': 3, "options": ["반직선", "반직선 아님", "알 수 없음", "해 없음"], 'title': '레이저 방향 지정', 'story': '[코덱스-L]: "크큭, 제법이군. 그렇다면 한쪽으로만 끝없이 뻗어 나가는 파괴 레이저의 명칭은 알고 있나? 대답하지 못하면 당장 방출시키겠다!"', 'qtext': '<strong>Q3.</strong> 점 A에서 시작하여 점 B 방향으로 끝없이 뻗어 나가는 선분을 기호로 나타낼 때, 이를 무엇이라 부르는가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 한쪽으로만 끝없이 뻗어 나가는 직선의 절반입니다.', 'ans_check': "ans.includes('반직선')"},
    ...
    {'qnum': 5, "options": ["120", "240", "118", "122"], 'title': '경보 태엽의 각도', 'story': '[코덱스-L]: "뭐야? 통신망에 쥐새끼가 숨어들었군. 하찮은 구식 메모리 따위! 시계 기어의 압력을 해제해 보시지. 이번엔 어림없을 거다!"', 'qtext': '<strong>Q5.</strong> 시계가 4시 정각을 가리킬 때, 시침과 분침이 이루는 작은 쪽의 각의 크기를 구하시오.', 'placeholder': "단위 없이 숫자만 입력 또는 '120도' 입력", 'error': '틀렸습니다. 시계의 1시간 간격은 30도입니다. 4시간의 각도를 계산해보세요.', 'ans_check': "ans === '120' || ans === '120도'"},
    {'qnum': 6, "options": ["맞꼭지각", "맞꼭지각 아님", "알 수 없음", "해 없음"], 'title': '제 2구역: 교차하는 빛줄기', 'story': '<span style=\"color: #60a5fa; text-shadow: 0 0 5px #3b82f6;\"><span style=\"color: #60a5fa; text-shadow: 0 0 5px #3b82f6;\"><span style=\"color: #60a5fa; text-shadow: 0 0 5px #3b82f6;\">[다빈치-메모리]</span></span></span>: "코덱스-L의 방해 전파가 강해지고 있습니다... 치직... 점 D를 지나는 직교 궤적을 찾으십시오... 서두르세요!"', 'qtext': '<strong>Q6.</strong> 두 직선이 교차할 때 생기는 교각 중, 마주 보는 두 각을 무엇이라 하는가?', 'placeholder': '정답 입력', 'error': '틀렸습니다. 마주 보고 솟은 꼭지점을 공유하는 각입니다.', 'ans_check': "ans === '맞꼭지각'"},
    {'qnum': 7, "options": ["같다", "같다 아님", "알 수 없음", "해 없음"], 'title': '광선의 대칭성', 'story': '[코덱스-L]: "닥쳐라, 고철 덩어리! 내가 직교 기호를 암호화해두었지. 인간의 빈약한 지식으로는 절대 해독할 수 없을 거다!"', 'qtext': '<strong>Q7.</strong> 맞꼭지각의 크기는 서로 어떠한가? (예: 같다, 다르다)', 'placeholder': '같다 또는 다르다 입력', 'error': '틀렸습니다. 마주 보는 각은 기하학적으로 항상 동일한 크기를 가집니다.', 'ans_check': "ans === '같다'"},
    ...
    {'qnum': 17, "options": ["SSS", "SAS", "AA", "ASA"], 'title': '세 기둥의 합동 락', 'story': '<span style=\"color: #60a5fa; text-shadow: 0 0 5px #3b82f6;\"><span style=\"color: #60a5fa; text-shadow: 0 0 5px #3b82f6;\"><span style=\"color: #60a5fa; text-shadow: 0 0 5px #3b82f6;\">[다빈치-메모리]</span></span></span>: "침착하십시오. 동위각과 엇각의 원리만 적용하면 녀석의 암호 체계를 붕괴시킬 수 있습니다!"', 'qtext': '<strong>Q17.</strong> 삼각형의 합동 조건 중 세 변의 길이가 각각 같을 때의 합동을 기호로 무엇이라 하는가? (예: SSS, SAS, ASA)', 'placeholder': '영문 대문자 세 글자 입력', 'error': '틀렸습니다. 변(Side) 세 개를 뜻하는 기호입니다.', 'ans_check': "ans === 'SSS' || ans === 'SSS합동'"},
    {'qnum': 18, "options": ["SSS", "SAS", "AA", "ASA"], 'title': '두 변과 사잇각의 락', 'story': '[코덱스-L]: "시스템 치명적 손상... 에러... 에러... 아직이다! 평행선 사이의 엇각 함정에 빠져보아라!"', 'qtext': '<strong>Q18.</strong> 삼각형의 두 변의 길이와 그 끼인각의 크기가 각각 같을 때의 합동 조건을 무엇이라 하는가?', 'placeholder': '영문 대문자 세 글자 입력', 'error': '틀렸습니다. 변(Side) 두 개와 각(Angle) 하나가 끼어 있는 기호입니다.', 'ans_check': "ans === 'SAS' || ans === 'SAS합동'"},
    {'qnum': 19, "options": ["SSS", "SAS", "AA", "ASA"], 'title': '한 변과 양 끝각의 락', 'story': '<span style=\"color: #60a5fa; text-shadow: 0 0 5px #3b82f6;\"><span style=\"color: #60a5fa; text-shadow: 0 0 5px #3b82f6;\"><span style=\"color: #60a5fa; text-shadow: 0 0 5px #3b82f6;\">[다빈치-메모리]</span></span></span>: "코덱스-L의 동력이 거의 바닥났습니다! 보조 각도를 계산해 마지막 결계의 압력을 낮추세요!"', 'qtext': '<strong>Q19.</strong> 삼각형의 한 변의 길이와 그 양 끝각의 크기가 각각 같을 때의 합동 조건을 무엇이라 하는가?', 'placeholder': '영문 대문자 세 글자 입력', 'error': '틀렸습니다. 각(Angle) 두 개 사이에 변(Side)이 끼어 있는 기호입니다.', 'ans_check': "ans === 'ASA' || ans === 'ASA합동'"},
